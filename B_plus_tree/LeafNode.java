...
public class LeafNode extends NonLeafNode{
	...
	public static boolean put(int key,Element el, LeafNode leaf) {
		...
		for(int i=0;i<size;i++) {
			if(key==leaf.ldata.get(i).getKey()) {
				System.out.println("중복 키 "+key+"은 허용되지 않습니다!");
				return false;
			}
		...
		if(delete_value!=Integer.MAX_VALUE) {
			if(isUnderflow(del_node)) {
				...
				if(del_node.nextNode!=null&&next.ldata.size()-1>=LeafNode.capacity/2&&del_node.parentNode==next.parentNode) {
					//int remove_key=next.ldata.get(0).getKey();
					del_node.ldata.add(next.ldata.remove(0));
					//빠져나간 key로 split_key가 바뀔경우
					//change_split_key(remove_key,next.ldata.get(0).getKey(),next.parentNode);
				...
				//previous node가 underflow 발생하지 않을 때
					...
					//int remove_key=del_node.ldata.get(0).getKey();
					del_node.ldata.add(0,previous.ldata.remove(previous.ldata.size()-1));
				//	change_split_key(remove_key,del_node.ldata.get(0).getKey(),del_node.parentNode);
			...
			//leaf의 left노드가 지워지면 key값변경
			else {
				check_keychange(del_node.ldata.get(0).getKey(),del_node.parentNode,del_node);
			}
			
		}
		else System.out.println(key+": 값이 존재하지 않습니다.");
	}
	...
	public static void merge(LeafNode previous,LeafNode next,LeafNode cur,int delete_value) {
		...
		//nextNode가 있는경우
		if(next!=null&&isCanMerge(next,cur)&&cur.parentNode==next.parentNode) {
			//2개의 노드 밖에없을 때 
		...
		//}
		//child에서 마지막 노드인경우
		else if(previous!=null&&isCanMerge(previous,cur)&&cur.parentNode==previous.parentNode) {
	...
	private static boolean isCanMerge(LeafNode cur,LeafNode other) {
		if(cur.ldata.size()+other.ldata.size()<=LeafNode.capacity-1) return true;
		else return false;
	}
