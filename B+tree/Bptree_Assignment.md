...
### Read  
**read**는 삽입, 삭제, 검색시 계속 이용하게 되는데, **index.dat**에 저장시킬 때는 다음의 구조를 이용하여 저장시키고 불러오게 했다. 각 줄을 설명하면, `L 3 1 3 2 123 6 1234`와 같이 저장되어있는데 **L**는 NonLeaf(**N**)인지 Leaf인지 구별시키고, 다음 **3**은 노드의 저장되어있는 size를 의미한다. 이후에는 **key,value** 순서로 저장되게 했다.  
NonLeaf의 **관계 연결**은 parent의 key size+1만큼 index.dat의 줄을 읽어들인다. e.x) key size가 3이면 다음 아래 4줄이 해당 줄의 child node를 담은 줄이다. 4줄을 읽을 때 **ArrayList**에 NonLeafNode 포인터 저장시켜 해당 노드들의 size만큼 아래의 줄을 읽어들여 관계를 형성해준다.(**next-previous**, **parent-child**) 그림으로 설명하면 아래와 같다.  
![image](memo_data.png)    
### Insert  
...
### Delete  
#### Leaf Node
**Delete**시에는 **Insert**할때와 같이 key가 포함되는 LeafNode를 Root 노드를 통해 찾아간다. 해당 키가 존재하면 삭제한다. 여기서 중요한 것은 노드의 크기는 n/2이상 n-1(n은 capacity)이하로 유지시켜야한다. 삭제 시 leaf-node가 underflow가 된다면 sibling 노드(부모가 같은 노드)로부터 가져올 수 있으면 key를 가져온다. 즉, sibling 노드가 underflow되지 않게 유지시킨다는 뜻이다. sibling node로부터 key를 가져왔을 때, 양쪽 sibling node가 underflow가 된다면 sibling node와 **병합**시킨다. sibling node와 병합하고 sibling node의 **leftmost** key키를 해당하는 **parent** node에서 제거해준다.  
#### Non-Leaf Node  
parent node에서 키를 제거했을 때, underflow가 발생하면 parent의 sibling node를 살펴본다. **Root-node**인 경우에는 **merge**하지 않는다. Root-node가 아니라면 sibling node와 병합을 한다. sibling node의 key 갯수가 **(n-1)/2+1**이상이면 parent의 sibling node로부터 leftmost key를 가져온다. 정확하게는 parent node의 parent로부터 key를 **underflow**된 parent node에 넣고, parent node의 **sibling** node의 leftmost key를 parent node의 parent로 올린다. **회전방식**을 이용한다. 만약 갯수가 **(n-1)/2+1**미만이면 sibling node와 병합을 하는데, 이때 parent node의 parent로부터 key를 하나 받는다. 조상 parent node의 key 갯수가 0이되면 **Root-node**는 parent-node가 된다.  
